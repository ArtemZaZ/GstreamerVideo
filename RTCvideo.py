...
class GLViewProperties:  # свойство viewport'а в текущем контексте OpenGl
    ...
    def setViewRectangle(self, rect):  # установить координаты viewport'a
        ...
        self.ConditionalRectangle = Rectangle(-1.0 * self.Scale, -1.0, 2.0 * self.Scale, 2.0)  # растягиваем видимые
        # координаты GL в зависимости от разрешения viewport'a

    def getScale(self):  # ф-ия для свойства
        return self.ViewPortRectangle.width / self.ViewPortRectangle.height

    def getkx(self):
        return self.ConditionalRectangle.width / self.ViewPortRectangle.width  # выдает размерный
        #  коэффициент(сколько пикселей по X в одной условной единице GL по X)

    def getky(self):
        return self.ConditionalRectangle.height / self.ViewPortRectangle.height  # выдает размерный
        #  коэффициент(сколько пикселей по Y в одной условной единице GL по Y)
...
initGLflag = False  # флаг инициализации opengl, все что связано с opengl в gstreamer должно быть


# определено глобально, т.к. когда кидаем функцию drawcallback в gst, сама ф-ия исполняется в gst модуле,
# который видит только глобальное пр-во имен, т.е. функцию drawcallback в класс не запихнешь
...
class OverlayTextureBlock:  # текстура накладываемая на экран в осях XY, обернутая в класс
    #  с методами, чтобы не писать все в drawcallback, по уже заявленной причине
    def __init__(self):
    ...
    def createTexture(self):  # генерация текстуры
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, 4, self.pixelX, self.pixelY, 0, GL_RGBA, GL_UNSIGNED_BYTE, self.image)  #
    ...
    def convertPixelToConditional(self):
        tempRec = Rectangle(
            (self.rectangle.x * self.glDrawProperties.kx + self.glDrawProperties.ConditionalRectangle.x),  #
    ...
    def drawOverlayTexture(self):  # рисует текстуру прямо на экране в осях XY (тут инвертирована ось Y)
        if self.texture:
            if self.fullScreen:  # если изображение надо растянуть по всему экрану,
                # то использовать координаты self.glDrawProperties.ConditionalRectangle
                glBindTexture(GL_TEXTURE_2D, self.texture)
                glBegin(GL_QUADS)
                glTexCoord2f(0.0, 0.0)
                glVertex3f(self.glDrawProperties.ConditionalRectangle.x, -self.glDrawProperties.ConditionalRectangle.y,
                           -1.0)
                glTexCoord2f(0.0, 1.0)
                glVertex3f(self.glDrawProperties.ConditionalRectangle.x, -(
                        self.glDrawProperties.ConditionalRectangle.y + self.glDrawProperties.ConditionalRectangle.height),
                           -1.0)
                glTexCoord2f(1.0, 1.0)
                glVertex3f(
                    self.glDrawProperties.ConditionalRectangle.x + self.glDrawProperties.ConditionalRectangle.width,
                    -(self.glDrawProperties.ConditionalRectangle.y + self.glDrawProperties.ConditionalRectangle.height),
                    -1.0)
                glTexCoord2f(1.0, 0.0)
                glVertex3f(
                    self.glDrawProperties.ConditionalRectangle.x + self.glDrawProperties.ConditionalRectangle.width,
                    -self.glDrawProperties.ConditionalRectangle.y, -1.0)
                glEnd()
            else:  # если нет
...
def drawCallback(GST_object, width, height, texture):
    ...
    if not initGLflag:  # если контекст opengl еще не проинициализирован
        initGL()  # инициализацию нужно производить именно сдесь, т.к. только эта ф-ия будет исполняться в модуле
        #  gst, собственно она и инициализирует opengl в нем
        glutInit(sys.argv)
        initGLflag = True  # контекст OpenGl нужно проинициализировать 1 раз. Текстуры тоже надо инициализировать в
        # callback ф-ии
        TEXTURE0.glDrawProperties.setViewRectangle(
            Rectangle(0, 0, width, height))  # Ставим разрешение Viewport'a в пикселях
...
class Video:
    def __init__(self, IP='127.0.0.1', RTP_RECV_PORT=5000, RTCP_RECV_PORT=5001, RTCP_SEND_PORT=5005,
                 overlay=drawCallback, codec="H264"):  # ip и порты по умолчанию
    ...
    def drawOverlay(self, path, x=0, y=0, scaleX=1.0, scaleY=1.0,
                    fullScreen=False):  # переходник между пользователем и GL
        global TEXTURE0
    ...
    ######################### GLUPLOAD ###########################
        ...
        ######################## GLCOLORCONVERT ############################
        self.glcolorconvert0 = Gst.ElementFactory.make("glcolorconvert",
                                                       "glcolorconvert0")  # преобразует цвета в формат цветов openGL
        ...
        self.glfilterapp0.connect("client-draw",
                                  self.drawcall)  # но у него есть небольшой косяк, если передавать в него переменную
        #  или ссылку как-то, то только глобвльную
    ...
    def linkElements(
            self):  # функция линковки элементов ###    (Примечание!!!) Scale можно делать в контексте самого OpenGL,
        #  в drawcallback просто домножая координаты на некоторую величину
